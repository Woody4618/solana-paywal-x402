...
function extractImageUrl(tweet: any, includes: any): string | undefined {
  try {
    ...
    if (mediaKeys.length && mediaIncludes.length) {
      ...
      const fromMedia =
        photo?.url ??
        items.find((m) => typeof (m as any).preview_image_url === 'string')?.preview_image_url;
...
async function uploadVideoAndReply(doneUrl: string, replyToTweetId: string): Promise<void> {
  try {
    ...
    console.error(
      '[x402] video upload failed, falling back to link',
...
async function runAnimateFlow(imageUrl: string, sourceTweetId: string): Promise<void> {
  ...
  const { ownerAta, recipientAta, ix } = await ensureAtaInstructions(
    connection,
    owner,
    recipient,
    mint
  ...
  const memoHex = paymentRequestToken ? sha256Hex(paymentRequestToken) : '';
  if (memoHex) {
    const memoData = Buffer.from(memoHex, 'utf8');
    ix.push(
  ...
  const amount =
    typeof paymentRequest.amount === 'bigint'
  ...
  try {
    const { blockhash: bh, lastValidBlockHeight } =
...
async function main() {
  ...
  const container =
    (res as any).data && !Array.isArray((res as any).data)
      ...
      : {
          data:
            (res as any).data?.data ?? (Array.isArray((res as any).data) ? (res as any).data : []),
  ...
  for (;;) {
    try {
      ...
      if (rl) {
        ...
        const resetIso =
          typeof rl.reset === 'number' ? new Date(rl.reset * 1000).toISOString() : 'unknown';
      ...
      if (err && typeof err === 'object' && 'code' in err && err.code === 429) {
        ...
        const rl = rlRaw ?? {};
        const resetMs =
          rl && typeof (rl as any).reset === 'number'
            ? (rl as any).reset * 1000
            : Date.now() + 60_000;
